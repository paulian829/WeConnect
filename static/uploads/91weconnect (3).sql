...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getAllFiles` (IN `uid` INT, IN `uemail` VARCHAR(100))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getAllFileSortedByName` ()  BEGIN
SELECT * FROM files ORDER BY FileName ASC;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `getAllFileSortedByType` ()  BEGIN
SELECT * FROM files ORDER BY FileType ASC;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `getAllFilesType` (IN `uid` INT, IN `uemail` VARCHAR(100))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getDone` ()  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getFileTask` (IN `uid` INT(11), IN `uTaskID` INT(11))  BEGIN
	SELECT * FROM files WHERE UploadedByID = uid AND taskID = uTaskID;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `getNFiles` (IN `uid` INT, IN `number` INT, IN `uemail` VARCHAR(100))  BEGIN
SELECT * FROM files WHERE (UploadedByID = uid OR Share_to_user = uemail)AND status=0 ORDER BY FileID DESC LIMIT number;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `getNumberOfFilesPassed` (IN `uid` INT, IN `estatus` VARCHAR(100))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getPassedForGradeChairman` ()  BEGIN
	SELECT * FROM tasks WHERE NOT TaskStatus = 'Pending Teachers' AND NOT TaskStatus = 'Pending Grade Chairman';
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `getPassedForPrincipal` ()  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getPending` (IN `uStatus` VARCHAR(100))  BEGIN
	SELECT * FROM tasks WHERE TaskStatus = uStatus ;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `getTaskByStatus` (IN `statusText` VARCHAR(100))  BEGIN
